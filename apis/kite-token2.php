...
// Initialize an empty array to store the cookies
$cookies = array();

// Add the option to save cookies in a file
curl_setopt($ch, CURLOPT_COOKIEJAR, 'cookies.txt');

curl_setopt($ch, CURLOPT_USERAGENT, 'Mozilla/5.0 (Windows NT 6.1; rv:11.0) Gecko/20100101 Firefox/11.0');
curl_setopt($ch, CURLOPT_HEADER, true); // Set CURLOPT_HEADER to true to capture headers in response
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true); // Set CURLOPT_RETURNTRANSFER to true to get the response as a string
...
// Store the complete response in $response variable

// Extract the cookies from the response headers and remove them from the response string
if (preg_match_all('/^Set-Cookie:\s*([^;]*)/mi', $response, $matches)) {
    foreach ($matches[1] as $cookie) {
//echo(json_encode($cookieArr).'<br><br><br>');
